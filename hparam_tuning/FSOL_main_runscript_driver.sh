...
# launch each FSOL combination of {model, dataset, log2eta, log10lmbda, seed}
for i in {0..8319}; do
    ...
    if [ "${least_busy_count}" -lt "${MAX_JOBS_PER_NODE}" ]; then
        srun --exclusive --ntasks 1 --mem-per-cpu=16G conda run -n afterburner python3 FSOL_main.py "${i}" &
